...
interface ItemSelectionModalProps {
  ...
  onOpenScanner: () => void;
  filterWarehouseId?: string; // ID kho để lọc vật tư hiện có
}
...
const ItemSelectionModal: React.FC<ItemSelectionModalProps> = ({ 
  ...
  // Logic lọc nghiêm ngặt theo yêu cầu
  const filteredItems = items.filter(item => {
    const matchesSearch = item.name.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          item.sku.toLowerCase().includes(searchTerm.toLowerCase());
    
    // Nếu có filterWarehouseId, CHỈ lấy item có tồn kho > 0 tại kho đó
          ...
          <div>
            ...
            {targetWarehouse && (
              <p className="text-xs text-blue-600 font-bold flex items-center mt-1 uppercase tracking-tighter">
                ...
                Chỉ hiển thị vật tư tại: {targetWarehouse.name}
              </p>
            )}
          </div>
                    ...
                    {filteredItems.map(item => (
                        <tr key={item.id} className="hover:bg-blue-50/30 transition-colors group">
                            ...
                            <td className="p-4 text-right">
                                <span className="font-black text-slate-700">{getDisplayStock(item).toLocaleString()}</span> 
