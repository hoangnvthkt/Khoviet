...
const RequestModal: React.FC<RequestModalProps> = ({ isOpen, onClose, request }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (request) {
        ...
        setNote('');
        setReqItems([{ itemId: items[0]?.id || '', qty: 1 }]); 
        setApprovedItems([]);
      }
    }
  ...
  const handleAddItem = () => {
    ...
    setReqItems([...reqItems, { itemId: items[0].id, qty: 1 }]);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl w-full max-w-5xl shadow-2xl flex flex-col max-h-[95vh] overflow-hidden relative">
        ...
        {/* Status Bar */}
        {!isEditable && (
            <div className={`px-6 py-2 text-white text-[10px] font-bold flex justify-between items-center ${
                request?.status === RequestStatus.PENDING ? 'bg-amber-600' :
